...
const OrderClient = (() => {
    ...
    function getOrCreateIdempotencyKey() {
        ...
        try {
            key = sessionStorage.getItem(IDEMPOTENCY_KEY_STORAGE);
        } catch (error) {
            console.warn("OrderClient: sessionStorage is unavailable.", error);
        }
        ...
        try {
            ...
            console.warn("OrderClient: could not persist idempotency key.", error);
        }
    ...
    function resetCheckoutAttempt() {
        try {
            ...
            console.warn("OrderClient: could not reset idempotency key.", error);
        }
    }
    ...
    function normalizeOrderData(orderDraft) {
        return {
            telegram_init_data: getTelegramInitData(),
            idempotency_key: getOrCreateIdempotencyKey(),
            customer_name: String(orderDraft.customer_name || "").trim(),
            phone: String(orderDraft.phone || "").trim(),
            email: String(orderDraft.email || "").trim(),
            contact_preferences: Array.isArray(orderDraft.contact_preferences)
                ? orderDraft.contact_preferences.slice()
                : [],
            payment_method: String(orderDraft.payment_method || "").trim(),
            note: String(orderDraft.note || "").trim(),
    ...
    async function createOrder(orderDraft) {
        if (!CORE_API_URL) {
            throw new Error("PREVIA Core Order API URL is not configured.");
        }

        const payload = normalizeOrderData(orderDraft || {});
        let response;
        ...
        if (!result || result.success !== true) {
            ...
            const orderError = new Error(orderMessage);
            orderError.code = orderCode;
            orderError.retryable = result?.retryable === true;
